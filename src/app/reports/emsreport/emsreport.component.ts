...
@Component({
  selector: 'app-emsreport',
  templateUrl: './emsreport.component.html',
  styleUrls: ['./emsreport.component.scss']
})
export class EmsreportComponent implements OnInit {
    ...
    constructor(private reportService: ReportService, private matDialog: MatDialog,
        private ngxLoader: NgxUiLoaderService,private EmsdashboardService:EmsdashboardService, private route: ActivatedRoute, private deviceManagerSer: DeviceMasterService,private _dashboardService: DashboardService,private transMngSer: TransformerManagerService) { }
    ...
    async ngOnInit() {
      ...
      this.deviceManagerSer.getEMSDeviceId(JSON.parse(localStorage.getItem('userData')).userid).subscribe((response:any) => {
        ...
        this.filters('DeviceID',Deviceids[0])
        this.deviceIds = Deviceids;
        this.selectedDevice = Deviceids[0];
      });
      this.ngxLoader.stop();
    }
    ...
    async filters(type: string, param: string) {
      ...
      param = param.split('(')[0];
       if(type == "DeviceID"){
        this.ngxLoader.startLoader('emsReport');
         this.device = param;
         this.transMngSer.getEnergyMeters(param).toPromise().then((response: any) => {
           let energyMeterId = [];
          response.result.forEach(function (value) {
          ...
          this.meterSerialNumber = energyMeterId;
          this.meterid = energyMeterId[0].split('(')[0];
          this.meteridforParameters = energyMeterId[0].split('(')[0];
          this.timeFilter = this.timeOptions[0];
          ...
          console.log("This is only response",response)
          console.log("This is the response.result of this array",response.result)
            if(response.result.length>0){
            this.dataSource = new MatTableDataSource(response.result);
            this.EmsdashboardService.getParameterConfig(energyMeterId[0].split('(')[0]).toPromise().then((response1: any) => {
            if(response1.result.length > 0){
              let Parameters = response1.result[0];
              let parameterList:any = ['Deviceid','DATE','energyMeterId'];
            ...
            console.log("kjhfsdksdfsdffdsdf",parameterList)
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;
            }
            });
            this.ngxLoader.stopLoader('emsReport');
           }  else {
            this.ngxLoader.stopLoader('emsReport');
           }
          });
       ...
       if(type == "energyMeter" ){
        ...
        this.transMngSer.getEnergyMeterReportData({"meterid":param,"duration":this.timeFilter,"deviceid":this.device,"range":this.customDateRange}).toPromise().then((response: any) => {
          console.log("This is only response 2",response)
          console.log("This is the response.result of this array 2",response.result)
          if(response.result.length>0){
           this.dataSource = new MatTableDataSource(response.result);
            this.EmsdashboardService.getParameterConfig(meterid.split('(')[0]).toPromise().then((response1: any) => {
            if(response1.result.length > 0){
              let Parameters = response1.result[0];
              let parameterList:any = ['Deviceid','DATE','energyMeterId'];
              delete Parameters['deletedFlag'];
            ...
            console.log("aksdkjsa",parameterList)
           this.heading =parameterList;
           this.dataSource.paginator = this.paginator;
           this.dataSource.sort = this.sort;
         ...
         if(param == "Custom" ){
          ...
          dialogRef.afterClosed().subscribe(value => {
            this.customDateRange = value;
              this.transMngSer.getEnergyMeterReportData({"meterid":this.meterid,"duration":param,"range":value,"deviceid":this.device}).toPromise().then((response: any) => {
               
                if(response.result.length>0){
                  this.dataSource = new MatTableDataSource(response.result);
                  this.EmsdashboardService.getParameterConfig(meterid.split('(')[0]).toPromise().then((response1: any) => {
                   console.log("this is response.result for date ",response)
                    if(response1.result.length > 0){
                      let Parameters = response1.result[0];
                      let parameterList:any = ['Deviceid','DATE','energyMeterId'];
                      delete Parameters['deletedFlag'];
                  ...
                  this.heading =Object.keys(response.result[0]);
                  this.dataSource.paginator = this.paginator;
                  this.dataSource.sort = this.sort;
                 } else {
                   this.dataSource = []
          ...
          this.transMngSer.getEnergyMeterReportData({"meterid":this.meterid,"duration":duration,"deviceid":this.device}).toPromise().then((response: any) => {
           console.log("hiiii else meterid",this.meterid)
           console.log("hiiii else duration",duration)
           console.log( "hiiii else deviceid",this.device)
           if(response.result.length>0){
              this.dataSource = new MatTableDataSource(response.result);
              this.EmsdashboardService.getParameterConfig(meterid).toPromise().then((response1: any) => {
                if(response1.result.length > 0){
                  let Parameters = response1.result[0];
                  let parameterList:any = ['Deviceid','DATE','energyMeterId'];
                  delete Parameters['deletedFlag'];
